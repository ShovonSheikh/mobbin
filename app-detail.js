...
function renderAppDetail(appName) {
  ...
  document.getElementById('screenCount').textContent = 
    `Total ${currentApp.screenshots.length} screens are saved`;
  ...
  screenshotsGrid.innerHTML = currentApp.screenshots.map((screenshot, index) => `
    <div class="screenshot-card" onclick="openImageModal('${screenshot.url}')">
      <img src="${screenshot.url}" alt="${screenshot.alt || 'Screenshot'}" class="screenshot-image">
      <div class="screenshot-actions">
        <button class="download-btn" onclick="event.stopPropagation(); downloadImage('${screenshot.url}', '${appName}', ${index})">
...
function downloadCurrentImage() {
  ...
  const appName = getAppNameFromUrl();
  const index = currentApp.screenshots.findIndex(s => s.url === currentImageUrl);
